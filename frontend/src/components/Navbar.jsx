    ...
    <>
      <nav className="border-b border-gray-800" style={{ background: "rgba(0,0,0,0.3)" }}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={onToggleTheme} className="p-2 rounded-lg hover:bg-gray-800 text-gray-400 transition-colors"
              title={theme === "dark" ? "切换浅色" : "切换深色"}>
              {theme === "dark" ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>
            {account ? (
              <button onClick={onDisconnect} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm border border-gray-700 text-gray-400 hover:text-white transition-colors">
                <Wallet className="w-4 h-4" />{account.slice(0, 6)}...{account.slice(-4)}<LogOut className="w-3 h-3" />
              </button>
            ) : (
              <button onClick={onConnect} className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium text-white"
