...
import { UploadCloud, File, X, CheckCircle2, Loader2, ShieldCheck } from "lucide-react";
...
export default function UploadDropzone({ onSuccess, apiBase }) {
  ...
  const onDrop = useCallback((accepted) => {
    setFiles(accepted.map((f) => Object.assign(f, { id: `${f.name}-${Date.now()}` })));
    setResult(null);
    setError(null);
    setProgress(0);
  }, []);
          ...
          {files.length === 0 ? (
            <>
              ...
              <p className="text-gray-300 text-base font-medium mb-1">
                {isDragActive ? "松开以添加文件" : "拖拽文件到此处"}
              </p>
              <p className="text-gray-600 text-sm">或点击选择 · 最大 50MB</p>
            </>
          ) : (
            <div className="w-full max-w-md">
              {files.map((f) => (
                <div key={f.id} className="flex items-center gap-4 p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.03)" }}>
                  <File className="w-8 h-8 shrink-0" style={{ color: "#06d6d6" }} />
      ...
      {result && (
        <div className="max-w-2xl mx-auto">
          ...
          <div className="text-center mt-4">
            <button onClick={removeFile} className="btn-secondary">继续上传新文件</button>
          </div>
        </div>
      )}
    </div>
